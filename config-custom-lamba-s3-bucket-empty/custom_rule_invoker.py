...
lamdainvoker = boto3.client('lambda')
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    try:
        ...
        response = lamdainvoker.invoke(FunctionName='arn:aws:lambda:us-east-1:427368090753:function:s3-detect-empty-bucket')
        print(response['ResponseMetadata']['HTTPHeaders']['x-amz-function-error'])
        return build_evaluation_from_config_item(configuration_item, 'NON_COMPLIANT', annotation = "Some error in function")
        
    except:
        return build_evaluation_from_config_item(configuration_item, 'COMPLIANT', annotation='No S3 buckets empty.')
